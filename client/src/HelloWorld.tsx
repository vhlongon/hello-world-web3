...
const abbreviateAddress = (address: string) => `${address.substring(0, 6)}...${address.substring(address.length - 4)}`;
...
const HelloWorld = () => {
  ...
  const addSmartContractListener = () => {
    helloWorldContract.events.UpdatedMessages(
      {},
      (error: Error, data: { returnValues: { oldStr: string; newStr: string } }) => {
        if (error) {
  ...
  return (
    <div id="container">
      ...
      <button id="walletButton" onClick={connectWalletPressed}>
        {walletAddress.length > 0 ? `Connected: ${abbreviateAddress(walletAddress)}` : <span>Connect Wallet</span>}
      </button>
