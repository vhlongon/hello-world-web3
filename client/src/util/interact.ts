...
export const helloWorldContract = new web3.eth.Contract(contract as AbiItem[], REACT_APP_CONTRACT_ADDRESS);
...
export const connectWallet = async () => {
  if (window?.ethereum) {
    try {
      ...
      const obj = {
        address: addressArray && addressArray.length > 0 ? addressArray[0] ?? '' : '',
